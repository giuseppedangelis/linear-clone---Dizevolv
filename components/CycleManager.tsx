          ...
          {cycles.map(cycle => {
            ...
            return (
              <div
                ...
                className="group bg-[#16161a] border border-[#1f1f23] rounded-xl p-6 hover:border-brand-primary/30 transition-all cursor-pointer shadow-lg"
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${isCurrent ? 'bg-brand-primary animate-pulse' : 'bg-gray-600'}`} />
                    <h3 className="font-semibold text-lg text-gray-100 group-hover:text-brand-primary transition-colors">
                      {cycle.name}
                    </h3>
                    <Badge variant={isCurrent ? 'orange' : 'gray'}>Cycle {cycle.number}</Badge>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between text-xs font-medium">
                    ...
                    <span className="text-brand-primary">{percent}% ({completed} / {total} issues)</span>
                  </div>
        ...
        <section className="mt-12">
          ...
          <div className="bg-[#16161a] border border-[#1f1f23] rounded-xl p-8 h-48 flex items-end gap-4">
            {[40, 65, 30, 85, 55, 90, 75].map((h, i) => (
              <div key={i} className="flex-1 bg-gradient-to-t from-brand-primary/20 to-brand-primary/10 border border-brand-primary/20 rounded-t-sm relative group" style={{ height: `${h}%` }}>
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-brand-primary px-2 py-0.5 rounded text-[10px] font-bold text-white">
                  {h} pts
                </div>
              </div>
            ))}
